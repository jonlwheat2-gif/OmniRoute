...
const PROVIDERS_PAGE_DATA_KEY = ["providers", "page-data"] as const;
const OAUTH_ENV_REPAIR_KEY = ["system", "env-repair"] as const;

/**
 * Client shell for the providers dashboard (Phase 1 RSC migration).
 *
 * The server page (`page.tsx`) renders the first paint: it fetches
 * `loadProviderPageData()` server-side (cookies forwarded) and passes the
 * result as `initialData`. This client seeds its state from that snapshot so
 * there is no client-side waterfall / skeleton flash, then uses TanStack Query
 * (key + staleTime) for the same data so mutations can invalidate it. Local
 * state remains the source of truth for optimistic updates (toggles, upserts).
 */
export default function ProvidersClient({ initialData }: { initialData: ProviderPageData }) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const [connections, setConnections] = useState<any[]>(initialData.connections);
  const [providerNodes, setProviderNodes] = useState<any[]>(initialData.providerNodes);
  const [ccCompatibleProviderEnabled, setCcCompatibleProviderEnabled] = useState(
    initialData.ccCompatibleProviderEnabled
  );
  const [blockedProviders, setBlockedProviders] = useState<string[]>(
    initialData.blockedProviders ?? []
  );
  const [expirations, setExpirations] = useState<any>(initialData.expirations);
  const [codexGlobalServiceMode, setCodexGlobalServiceMode] = useState<CodexGlobalServiceMode>(
    getCodexGlobalServiceMode(initialData.settings)
  );
  const [loading, setLoading] = useState(false);
  // `showAllProviders` gates the first-provider hint; the value is never toggled
  // in-page (kept as state to preserve the original `setShowAllProviders` seam).
  ...
  // Server-fetched snapshot is seeded via `initialData` (RSC first paint).
  // This query keeps the same key/staleTime so invalidations (after toggles,
  // import-from-file, add-compatible) can refresh the page; the server data is
  // considered fresh for STALE_MS so the client does not re-fetch on mount.
  const { data: pageData } = useQuery({
    queryKey: PROVIDERS_PAGE_DATA_KEY,
    queryFn: () => loadProviderPageData(),
    initialData,
    // The server already fetched this snapshot for this request — treat it as
    // fresh so hydration does not trigger a duplicate client fetch on mount.
  ...
  useEffect(() => {
    if (!pageData) return;
    setConnections(pageData.connections);
    setProviderNodes(pageData.providerNodes);
    setCcCompatibleProviderEnabled(pageData.ccCompatibleProviderEnabled);
    if (pageData.expirations) setExpirations(pageData.expirations);
    if (pageData.blockedProviders) setBlockedProviders(pageData.blockedProviders);
    setCodexGlobalServiceMode(getCodexGlobalServiceMode(pageData.settings));
    setOpenRouterProviderStats(pageData.openRouterProviderStats);
    setLoading(false);
  }, [pageData]);
  ...
  const { data: oauthEnvRepairData } = useQuery({
    queryKey: OAUTH_ENV_REPAIR_KEY,
    queryFn: async () => {
  ...
  const oauthEnvRepairStatus = oauthEnvRepairData ?? null;
  ...
  const handleRepairEnv = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: OAUTH_ENV_REPAIR_KEY });
    } catch (error) {
  ...
  const handleToggleProvider = async (providerId: string, authType: string, newActive: boolean) => {
    ...
    // Keep the TanStack snapshot in sync with the optimistic local state.
    await queryClient.invalidateQueries({ queryKey: PROVIDERS_PAGE_DATA_KEY });
  };
